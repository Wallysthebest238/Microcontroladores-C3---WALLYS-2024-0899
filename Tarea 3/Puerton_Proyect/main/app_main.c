...
#define DEF_WIFI_PASS "PlusUltra"
...
static void save_config(const app_config_t *cfg)
{
    ...
    if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &nvsh) == ESP_OK) {
        ...
        ESP_LOGE(TAG, "Error guardando NVS");
    }
}
...
static void vTimerCallback(TimerHandle_t xTimer)
{
    ...
    /* Si ambos finales activos -> error */
...
int Func_WAIT()
{
    ...
    while (1) {
        ...
        /* si ambos finales activos en espera error */
        if (in.LSA && in.LSC) {
            EST_SIG = EST_ERR;
            return EST_SIG;
        }
